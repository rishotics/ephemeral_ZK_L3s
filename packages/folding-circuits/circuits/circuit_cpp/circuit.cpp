...
uint get_total_signal_no() {return 308;}

uint get_number_of_components() {return 13;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 275;}
...
for (uint i = 0; i < 5; i++) {
std::string new_cmp_name = "checkEqual"+ctx->generate_position_array(aux_dimensions, 1, i);
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "checkLessThan";
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[5]); // line circom 19
while(Fr_isTrue(&expaux[0])){
{
...
// load src
Fr_sub(&expaux[1],&lvar[1],&circuitConstants[1]); // line circom 32
Fr_add(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&expaux[1])) + 13)],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[1])) + 0)]].signalStart + 0]); // line circom 32
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&circuitConstants[6],&signalValues[mySignalStart + 17]); // line circom 36
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + 2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[5]].signalStart + 0]); // line circom 42
